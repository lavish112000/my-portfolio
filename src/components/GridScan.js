...
const DEFAULT_FACE_API_SCRIPT_URL =
...
const loadExternalScript = (src) => {
  ...
  const cached = externalScriptPromises.get(src);
  if (cached) return cached;

  const existing = document.querySelector(`script[src="${src}"]`);
  if (existing) {
    const p = Promise.resolve();
    externalScriptPromises.set(src, p);
    return p;
  }
...
const GridScan = ({
  ...
  faceApiScriptUrl = DEFAULT_FACE_API_SCRIPT_URL,
  sensitivity = 0.55,
  lineThickness = 1,
  linesColor = '#392e4e',
  scanColor = '#FF9FFC',
  ...
  useEffect(() => {
    if (!enablePointerTracking) return undefined;
  ...
  }, [uiFaceActive, snapBackDelay, scanOnClick, enableGyro, enablePointerTracking]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!window.faceapi) {
          await loadExternalScript(faceApiScriptUrl);
        }
  ...
  }, [modelsPath, faceApiScriptUrl, enableWebcam]);
